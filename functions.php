...
// Ensure that Objekt-ID search actually limits the archive results to the matched objects.
add_action( 'pre_get_posts', function ( $query ) {
	if ( is_admin() || ! $query->is_main_query() ) {
		return;
	}

	// Only affect ImmoMakler object archives / search.
	$post_type = $query->get( 'post_type' );
	if ( $post_type && $post_type !== 'immomakler_object' ) {
		return;
	}
	...
	// Reuse ImmoMakler's normalization logic.
	if ( class_exists( '\ImmoMakler\Helpers\String_Helper' ) ) {
		$normalized = \ImmoMakler\Helpers\String_Helper::normalize_objektnr_extern( $objekt_id );
	} else {
		$normalized = $objekt_id;
	}
...
add_action( 'immomakler_search_form_after_ranges', function () {
	...
	// Objekt-ID (property ID) field - integrates with core "objekt-id" search.
	$objekt_id = '';
	if ( isset( $_GET['objekt-id'] ) ) {
		$objekt_id = sanitize_text_field( (string) wp_unslash( $_GET['objekt-id'] ) );
	} elseif ( isset( $_POST['objekt-id'] ) ) {
	...
	echo '<div class="range-label">' . esc_html__( 'Objekt-ID', 'immomakler-child-skin' ) . '</div>';
	echo '<div class="woonwoon-minmax">';
